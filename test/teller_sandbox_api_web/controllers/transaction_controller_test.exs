defmodule TellerSandboxApiWeb.TransactionControllerTest do
  ...
  describe "all" do
    test "incorrect params is a 404", %{conn: conn} do
      expect(DateMock, :utc_today, fn -> ~D[2020-11-23] end)
  ...
  describe "get" do
    ...
    test "We get the transaction returned", %{conn: conn} do
      ...
      response =
        ...
        |> get(
          Routes.transaction_path(conn, :get, "test_acc_GEZDGNBVGY3TQOJQ", "test_txn_GIYTCOJXGY4DCMRY"),
        %{
          "account_id" => "test_acc_GEZDGNBVGY3TQOJQ",
          "transaction_id" => "test_txn_GIYTCOJXGY4DCMRY"
        })
        |> json_response(200)
      ...
      assert response ==           %{
        ...
        "status" => "posted",
        "id" => "test_txn_GIYTCOJXGY4DCMRY",
        "links" => %{
          "account" => "http://localhost:4000/accounts/test_acc_GEZDGNBVGY3TQOJQ",
          "self" => "http://localhost:4000/accounts/test_acc_GEZDGNBVGY3TQOJQ/transactions/test_txn_GIYTCOJXGY4DCMRY"
        },
        "running_balance" => "21310.77",
        "type" => "card_payment"
      }
